...
export function buildLazySubagentsHelp(): string {
  return [
    ...
    "  lazy_subagents action=workflow steps=[{id,agent,prompt,taskSummary?,dependsOn?,cwd?}, ...] [maxConcurrency=<n>] [completionPolicy=<policy>] [title=<title>]",
    ...
    "  lazy_subagents action=workflow steps=[{id:\"research\",agent:\"scout\",prompt:\"Inspect the package layout\"},{id:\"plan\",agent:\"reviewer\",dependsOn:[\"research\"],prompt:\"Use {{research.summary}} to draft a refactor plan\"}] maxConcurrency=2",
    "",
